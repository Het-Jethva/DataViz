...
export const processChartData = (data, xAxis, yAxis, chartType = "line") => {
  ...
  if (!columns.includes(xAxis) || !columns.includes(yAxis)) {
    console.log('processChartData: Invalid axes provided, returning null', { xAxis, yAxis, availableColumns: columns })
    return null
  }
  ...
  try {
    ...
    if (chartType === "pie" || chartType === "doughnut") {
      ...
      processedData.datasets = [{
        ...
        borderColor: generateColors(Object.keys(aggregatedData).length).map(color => color.replace('0.8', '1')),
        borderWidth: 1
      }]
    } else {
...
export const process3DData = (data, xAxis, yAxis, zAxis) => {
  ...
  if (!columns.includes(xAxis) || !columns.includes(yAxis) || !columns.includes(zAxis)) {
    console.log('process3DData: Invalid axes provided, returning null', { xAxis, yAxis, zAxis, availableColumns: columns })
    return null
  }
...
export const getChartOptions = (chartType, title = "Data Visualization", xAxisLabel = "X Axis", yAxisLabel = "Y Axis") => {
  const baseOptions = {
    responsive: true,
...
export const validateAxisSelection = (data, xAxis, yAxis, chartType) => {
  ...
  if (chartType === "scatter") {
    if (!isXNumeric) {
      return { isValid: false, error: "X-axis must be numeric for scatter plots" }
    }
    if (!isYNumeric) {
      return { isValid: false, error: "Y-axis must be numeric for scatter plots" }
    }
  }
  ...
  if (["line", "area"].includes(chartType)) {
    if (!isYNumeric) {
      return { isValid: false, error: "Y-axis must be numeric for line/area charts" }
    }
  }
  ...
  if (chartType === "pie" || chartType === "doughnut") {
    if (!isYNumeric) {
      return { isValid: false, error: "Y axis must be numeric for pie/doughnut charts" }
    }
  }
  ...
  if (chartType === "bar") {
    if (!isYNumeric && !isXNumeric) {
      return { isValid: false, error: "For bar charts, at least one axis should be numeric" }
    }
  }
...
export const validate3DAxisSelection = (data, xAxis, yAxis, zAxis) => {
  ...
  const isNumericX = sampleValuesX.every(value => 
    !isNaN(value) && value !== null && value !== undefined && value !== ""
  )
  const isNumericY = sampleValuesY.every(value => 
    !isNaN(value) && value !== null && value !== undefined && value !== ""
  )
  const isNumericZ = sampleValuesZ.every(value => 
    !isNaN(value) && value !== null && value !== undefined && value !== ""
  )
  
  if (!isNumericX || !isNumericY || !isNumericZ) {
    return { isValid: false, error: "All axes must be numeric for 3D charts" }
  }
...
export const validateChartData = (data, chartType) => {
  ...
  if ((chartType === "pie" || chartType === "doughnut") && data.length < 2) {
    return { isValid: false, error: "Pie/doughnut charts require at least 2 data points" }
  }
...
export const validateYAxisData = (data, yAxis, chartType = "line") => {
  if (!data || data.length === 0) {
    return { isValid: false, error: "No data available", numericValues: 0, totalValues: 0 }
  }
  
  if (!yAxis) {
    return { isValid: false, error: "Y-axis not selected", numericValues: 0, totalValues: 0 }
  }
